...
class Trainer(object):
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            # Clamp using tanh.
            noise = torch.clamp(ch.nn.functional.hardtanh(perturbations_mean) * eps, -1 * self._epsilon, self._epsilon)
            ...
            # We don't sample and use deterministic adversary policy here.
            perturbations_mean, _ = self.attack_policy_network(last_states)
            noise = torch.clamp(self.perturb_obs_fgsm(perturbations_mean, last_states), -1 * self._epsilon, self._epsilon)
